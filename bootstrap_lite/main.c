...
#define HEN_REPO_URL "https://github.com/TecnoRodriguez/TecnoRodriguezhenlo"
#define VDEP_VPK_FNAME "vdep.vpk"
#define AUTOP_VPK_FNAME "autop.vpk"
#define VDEP_VPK_FNAME "vdep.vpk"
...
const char* menu_items[OPTION_COUNT] = { "Exit", "Install henkaku", "Install VitaDeploy", "Install vitashell", "Install autoplugin", "Replace NEAR with VitaDeploy", "Reset taihen config.txt", "Exit and mount sd2vita to ux0" };
...
int install_vitashell_default() {
    sceIoMkdir(TEMP_UX0_PATH, 0777);
    COLORPRINTF(COLOR_CYAN, "Downloading vitashell\n");
    net(1);
    int res = download_file(HEN_REPO_URL VDEP_VPK_FNAME, TEMP_UX0_PATH VDEP_VPK_FNAME, TEMP_UX0_PATH VDEP_VPK_FNAME "_tmp", 0);
    if (res < 0) {
        if ((uint32_t)res == 0x80010013)
...
int install_autoplugin_default() {
    sceIoMkdir(TEMP_UX0_PATH, 0777);
    COLORPRINTF(COLOR_CYAN, "Downloading autoplugin\n");
    net(1);
    int res = download_file(HEN_REPO_URL VDEP_VPK_FNAME, TEMP_UX0_PATH VDEP_VPK_FNAME, TEMP_UX0_PATH VDEP_VPK_FNAME "_tmp", 0);
    if (res < 0) {
        if ((uint32_t)res == 0x80010013)
